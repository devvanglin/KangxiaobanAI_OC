...
func (r *FamilyRepository) BoundElderIDs(ctx context.Context, userID uint) ([]uint, error) {
...
// Bind 建立/幂等创建绑定。
func (r *FamilyRepository) Bind(ctx context.Context, userID, elderID uint) error {
	return r.db.WithContext(ctx).FirstOrCreate(&model.FamilyElder{UserID: userID, ElderID: elderID},
