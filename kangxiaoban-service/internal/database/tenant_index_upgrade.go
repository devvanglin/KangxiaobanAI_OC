...
func ensureTenantScopedIdentityIndexes(db *gorm.DB) error {
	if db.Dialector.Name() == "sqlite" {
		if err := db.Exec("DROP INDEX IF EXISTS idx_roles_code").Error; err != nil {
			return err
		}
	...
	if db.Migrator().HasIndex(&model.Role{}, "idx_roles_code") {
		if err := db.Migrator().DropIndex(&model.Role{}, "idx_roles_code"); err != nil {
			return err
		}
	}
	if db.Migrator().HasIndex(&model.User{}, "idx_users_username") {
		if err := db.Migrator().DropIndex(&model.User{}, "idx_users_username"); err != nil {
			return err
		}
	}
	if !db.Migrator().HasIndex(&model.Role{}, "uk_roles_tenant_code") {
		if err := db.Exec("CREATE UNIQUE INDEX uk_roles_tenant_code ON roles(tenant_id, code, deleted_at)").Error; err != nil {
			return err
		}
	}
