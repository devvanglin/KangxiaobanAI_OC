...
func New(db *gorm.DB, cfg *config.Config, hub *ws.Hub, iotSvc *iot.IotService,
	...
	authed.Use(middleware.Audit(auditRepo))
	{
		...
		authed.POST("/admin/storage/buckets/:bucket/objects", perm("admin:all"), storageHandler.UploadObject)
		...
		authed.GET("/admin/ai/tools", perm("admin:all"), aiAdminHandler.ToolsList)
		authed.GET("/admin/ai/sandbox", perm("admin:all"), aiAdminHandler.Sandbox)
		authed.PUT("/admin/ai/sandbox", perm("admin:all"), aiAdminHandler.UpdateSandbox)
		authed.POST("/admin/ai/sandbox/probe", perm("admin:all"), aiAdminHandler.SandboxProbe)
		...
		authed.GET("/admin/ai/prompts", perm("admin:all"), aiAdminHandler.AdminPromptList)
