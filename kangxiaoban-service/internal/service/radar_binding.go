...
func SyncMillimeterRadarBindingForElder(ctx context.Context, db *gorm.DB, elder *model.Elder) {
	...
	var bed model.Bed
	if err := db.WithContext(ctx).Where("id = ?", *elder.BedID).First(&bed).Error; err != nil {
		return
	}
	var room model.Room
	if err := db.WithContext(ctx).Where("id = ?", bed.RoomID).First(&room).Error; err != nil || room.RoomNo == "" {
		return
	}
	db.WithContext(ctx).Model(&model.IotDevice{}).
...
func SyncElderForRadarRoom(ctx context.Context, db *gorm.DB, device *model.IotDevice) {
	...
	err := db.WithContext(ctx).
		Joins("JOIN beds ON beds.elder_id = elders.id AND beds.status = ?", "occupied").
	...
	db.WithContext(ctx).Model(&model.IotDevice{}).Where("id = ?", device.ID).
		Update("elder_id", elder.ID)
