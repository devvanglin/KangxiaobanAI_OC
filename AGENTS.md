...
| `component/wide/WideAssessmentAgentPage.ets` | native HDS voice/text assessment, microphone permission fallback, progress, resume, completion handoff; question at top-left, per-question no-answer countdown at top-right, and the central Grok Ball expression stage (`rawfile/grokball/`, MIT, ArkWeb + `window.stage.setEmotion` bridge) whose emotion follows the assessment phase (01 connecting, 39 TTS speaking, 31 elder ASR speaking, 40 answer timeout, 35 listening, 03 urgent, 30 recognizing/generating report, 33 done, 34 error) |
